...
Last modified on 4th March 2020.
	compilation: g++ -O3 -o metasim_nd metacommunity_simulator_temporal_wave_neighbourdisp.cpp -lgsl -lgslcblas -lm
*************************************************************************************/
// Libraries
#include <stdio.h>
#include <iostream>
...
int main(){
	...
	// OUTPUT
	char nomfo[256];
	sprintf(nomfo,"burn-int_S3_d%d_J%d_m%d_ldd%d_A%d_sig%d_env%d_%d_%d_%d.txt",int(d*100),iK,int(m*100),int(10*LDD),int(10*A),int(sig[0]*1000),int(param_env[0]*100),int(param_env[1]*100),int(param_env[2]*100),int(param_env[3]*100));
	ofstream outS(nomfo);
	...
	sprintf(nomfo,"metacomt3_d%d_J%d_m%d_ldd%d_A%d_sig%d_env%d_%d_%d_%d.txt",int(d*100),iK,int(m*100),int(10*LDD),int(10*A),int(sig[0]*1000),int(param_env[0]*100),int(param_env[1]*100),int(param_env[2]*100),int(param_env[3]*100));
	ofstream outc(nomfo);
	sprintf(nomfo,"environmentt3_d%d_J%d_m%d_ldd%d_A%d_sig%d_env%d_%d_%d_%d.txt",int(d*100),iK,int(m*100),int(10*LDD),int(10*A),int(sig[0]*1000),int(param_env[0]*100),int(param_env[1]*100),int(param_env[2]*100),int(param_env[3]*100));
	ofstream oute(nomfo);
	sprintf(nomfo,"St3_d%d_J%d_m%d_ldd%d_A%d_sig%d_env%d_%d_%d_%d.txt",int(d*100),iK,int(m*100),int(10*LDD),int(10*A),int(sig[0]*1000),int(param_env[0]*100),int(param_env[1]*100),int(param_env[2]*100),int(param_env[3]*100));
	...
	sprintf(nomfo,"betat3_d%d_J%d_m%d_ldd%d_A%d_sig%d_env%d_%d_%d_%d.txt",int(d*10),iK,int(m*100),int(10*LDD),int(10*A),int(sig[0]*1000),int(param_env[0]*100),int(param_env[1]*100),int(param_env[2]*100),int(param_env[3]*100));
	ofstream outb(nomfo);
	for (int i=0;i<ns;i++){
		for (int j=0;j<12;j++){
			outb<<stats_beta[i][j]<<" ";
